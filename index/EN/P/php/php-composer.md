...
| Версия Composer | Минимальная версия PHP | Примечание |
| --- | --- | --- |
| **1.x** | **5.3.2+** | Официально поддержка завершена. |
| **2.0.x – 2.1.x** | **5.3.2+** | Последние версии Composer 2.x, поддерживающие PHP 5.3. |
| **2.2 (LTS)** | **5.3.2+** | **Рекомендуемый выбор.** Версия с долгосрочной поддержкой для старых проектов. |
| **2.3+** | **7.2.5+** | Современные версии, не поддерживают PHP 5.3. |

---

## Установка 

* Скачать его можно напрямую:
```bash
wget https://getcomposer.org/download/2.2.25/composer.phar
```

* Игнорируйте проверки. 

При запуске на современном PHP всегда используйте флаг --ignore-platform-reqs. Он отключает проверку того, что «Composer запущен на PHP 8.x, а проект хочет 5.3».
```shell
php8.2 /path/to/composer.phar update --ignore-platform-reqs --no-audit
```
...
* Если же в папке my-app/library/my-library/ клон библиотеки из GIT
